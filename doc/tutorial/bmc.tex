...
contained in file \filename{modulo8.smv}):
\begin{alltt}
MODULE main
VAR 
  y : 0..15;
...
To this purpose, we add the following specification to file \filename{modulo8.smv}:
\begin{alltt}
LTLSPEC G ( y=4 -> X y=6 )
\end{alltt}
and we instruct \nusmv to run in BMC by using command-line option \code{-bmc}:
\begin{alltt}
\shellprompt \shelltext{\nusmvtxt -bmc modulo8.smv}
...
\begin{alltt}
\shellprompt \shelltext{\nusmvtxt -bmc -bmc\_length 4 modulo8.smv}
...
specification of file \filename{modulo8.smv} as follows:
\begin{alltt}
LTLSPEC !G F (y = 2)
\end{alltt}
%\noindent
...
Let us modify file \filename{modulo8.smv} by replacing the LTL specification
